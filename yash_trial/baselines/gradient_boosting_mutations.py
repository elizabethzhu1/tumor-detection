"""
Gradient Boosting Classifier for Tumor Type Prediction
...
def plot_feature_importances(importances, feature_names, save_path):
    """Save a bar chart of the feature importances."""
    fig, ax = plt.subplots(figsize=(7, 5))
    indices = np.argsort(importances)[::-1]
    sorted_importances = importances[indices]
    sorted_names = [feature_names[i] for i in indices]
    
    colors = plt.cm.Blues(np.linspace(0.8, 0.4, len(importances)))
...
def main():
    ...
    with open(results_dir / "gb_metrics.json", "w") as f:
        json.dump(metrics, f, indent=4)
    print(f"Saved metrics to: {results_dir / 'gb_metrics.json'}")

    # Plot and save confusion matrix
    plot_confusion_matrix(cm, class_names, results_dir / "gb_confusion_matrix.png")
    print(f"Saved confusion matrix plot to: {results_dir / 'gb_confusion_matrix.png'}")

    # Plot and save feature importances
    plot_feature_importances(clf.feature_importances_, MUTATION_TYPES, results_dir / "gb_feature_importances.png")
    print(f"Saved feature importances plot to: {results_dir / 'gb_feature_importances.png'}")
