...
maf_processing.py        # GDC MAF parsing + synthetic data generator
...
outputs/
  spectra_counts.csv      # Per-sample 96-d SNV count spectra
  results.json            # Metrics and learned weights from last run
```

## Run the MVP

```bash
python run_pipeline.py            # synthetic data, no COSMIC file needed
```
...
python run_pipeline.py \
  --no-synthetic \
  --download-tcga-mafs \
  --cosmic-path data/COSMIC_v3.4_SBS_GRCh38.tsv
```
...
bottleneck weights are written to `outputs/results.json`.
...
- `--max-files-per-project 20` limits downloads for quick test runs.
...
- `--spectra-out path/to/counts.csv` changes where the 96-d counts are saved.
...
`--cosmic-path`. If omitted, the code uses synthetic placeholder signatures so
the neural-network path still runs.
